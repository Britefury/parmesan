...
from parmesan.layers.ladderlayers import RasmusInit, DenseLadderLayer, ConvPoolLadderLayer, ReshapeLadderLayer, InputLadderLayer, build_ladder_ae
import os, sys, time
import uuid
import parmesan
...
parser.add_argument("-lambdas", type=str,
                    default='100,10,0.1,0.1,0.1,0.1')
...
x_valid = x_valid.astype('float32').reshape((-1,1,28,28))
...
ll0 = InputLadderLayer(shape=(None, 1,28,28), cost_weight=100.0)
ll1 = ConvPoolLadderLayer(num_filters_in=1, num_filters_out=32, filter_size=(5,5), pool_size=(2,2), nonlinearity=unit, init=init, cost_weight=10.0)
ll2 = ConvPoolLadderLayer(num_filters_in=32, num_filters_out=32, filter_size=(3,3), pool_size=None, nonlinearity=unit, init=init, cost_weight=1.0)
ll3 = ConvPoolLadderLayer(num_filters_in=32, num_filters_out=32, filter_size=(3,3), pool_size=(2,2), nonlinearity=unit, init=init, cost_weight=0.1)
ll4 = ReshapeLadderLayer(shape_in=(32,4,4), shape_out=(512,))
ll5 = DenseLadderLayer(num_units_in=512, num_units_out=256, nonlinearity=unit, init=init, cost_weight=0.1)
ll6 = DenseLadderLayer(num_units_in=256, num_units_out=10, nonlinearity=softmax, init=init, cost_weight=0.1)
